...
import { ProveedorPago } from "./pagos/proveedorPago";
import { mercadoPagoProvider } from "./pagos/mercadoPagoProvider";
 ...
 */
function obtenerProveedorPago(): ProveedorPago {
...
app.post("/calcular", (req, res) => {
  try {
    ...
    const input: CalculoInput = {
      ...
      regimen_fiscal: regimen_fiscal as CalculoInput["regimen_fiscal"],
    ...
    const resultado = calcularReservaAirbnb(input);
...
app.post("/reporte", (req, res) => {
  try {
    ...
    const resumen = {
      ...
      salida: resultado,
      disclaimer:
        "Este reporte ofrece cálculos aproximados con fines informativos. No constituye asesoría fiscal, contable ni legal.",
    };
...
app.post("/reporte-pdf", (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="reporte-reserva.pdf"'
    );
    ...
    doc.text(
      `Comisión Airbnb estimada: ${resultado.comision_airbnb_reserva} MXN`
    );
    ...
    doc.text(
      `Base estimada para impuestos: ${resultado.base_impuestos} MXN`
    );
    doc.text(
      `Impuestos estimados (aprox.): ${resultado.impuestos_estimados_reserva} MXN`
    );
    doc.moveDown();
    ...
    doc
      .fontSize(9)
      .text(
        "Los resultados no sustituyen el cálculo realizado por un Contador Público Certificado ni consideran todas las variables de la legislación vigente."
      );
...
app.post("/crear-checkout", async (req, res) => {
  try {
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description:
                "Incluye desglose de ingresos, gastos e impuestos estimados. Uso informativo, no asesoría fiscal.",
            },
          },
          quantity: 1,
        },
    ...
    return res.status(500).json({
      ok: false,
      error:
        "No se pudo iniciar el pago del reporte. Intenta de nuevo más tarde.",
    });
  }
});
...
app.post("/crear-sesion-reporte", async (_req, res) => {
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: "http://192.168.1.79:3000/?payment=cancel",
    });
