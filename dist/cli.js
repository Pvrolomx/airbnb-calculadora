...
if (args.length < 7) {
    console.error("Uso: node dist/cli.js tarifa_noche noches tarifa_limpieza regimen_fiscal gasto_limpieza_real gasto_consumibles gasto_comisiones_otras");
    process.exit(1);
}
const [tarifa_noche_str, noches_str, tarifa_limpieza_str, regimen_fiscal_str, gasto_limpieza_real_str, gasto_consumibles_str, gasto_comisiones_otras_str,] = args;
const input = {
    tarifa_noche: Number(tarifa_noche_str),
    noches: Number(noches_str),
    tarifa_limpieza: Number(tarifa_limpieza_str),
    regimen_fiscal: regimen_fiscal_str,
...
try {
    const output = (0, calculo_1.calcularReservaAirbnb)(input);
